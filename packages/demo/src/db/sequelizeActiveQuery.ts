...
export class SequelizeActiveQuery<T extends Model> implements QueryInterface<T> {
    static create<T extends Model>(modelClass: M<T>) {
        return new SequelizeActiveQuery(modelClass)
    }
    ...
    clone(): this {
        ...
        const result = new SequelizeActiveQuery<T>(this.modelClass)
